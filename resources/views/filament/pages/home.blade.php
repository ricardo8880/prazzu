    ...
    <div data-home-layout="operational" class="pz-central-page" x-data="pzHomeResolutionModal()" x-on:keydown.escape.window="closeResolutionModal()" x-on:keydown.ctrl.enter.window.prevent="resolutionOpen && confirmCompletion()">
        ...
        <section class="pz-central-day-grid" aria-label="Resumo crítico da Central do Dia">
            @foreach($centralDiaCards as $card)
                <button type="button" class="pz-central-day-card pz-tone-{{ $card['tone'] ?? 'slate' }}" :class="isActiveSummaryFilter('{{ $card['filter'] ?? '' }}', '{{ $card['target'] ?? 'priority' }}') ? 'is-filter-active' : ''" @click="applySummaryFilter('{{ $card['filter'] ?? '' }}', '{{ $card['target'] ?? 'priority' }}', '{{ $card['label'] ?? 'Filtro' }}')">
                    ...
                    <em x-text="isActiveSummaryFilter('{{ $card['filter'] ?? '' }}', '{{ $card['target'] ?? 'priority' }}') ? 'Filtro aplicado' : 'Filtrar lista →'"></em>
                </button>
            @endforeach
        </section>
        ...
        <script>
            function pzHomeResolutionModal() {
                return {
                    ...
                    countdownTimer: null,
                    priorityItems: @js($filaPrioridadeCompleta->values()),
                    clientItems: @js($clientesMaiorRisco->values()),
                    priorityFilter: '',
                    priorityFilterLabel: '',
                    clientFilter: '',
                    clientFilterLabel: '',
