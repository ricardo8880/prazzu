...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .ticket-actions { display: grid; gap: 12px; margin-bottom: 18px; }
        .new-ticket-box { border: 1px solid #cfe0ff; border-radius: 10px; background: linear-gradient(135deg, #f5f9ff, #fff); padding: 14px; display: grid; gap: 10px; }
        .new-ticket-box summary { cursor: pointer; font-weight: 950; color: #0f3f93; display: flex; align-items: center; justify-content: space-between; gap: 10px; list-style: none; }
        .new-ticket-box summary::-webkit-details-marker { display: none; }
        .new-ticket-form { display: grid; gap: 10px; padding-top: 10px; }
        .new-ticket-form input, .new-ticket-form textarea, .new-ticket-form select { width: 100%; border: 1px solid var(--pc-line); border-radius: 8px; padding: 11px 12px; background: #fff; color: var(--pc-text); }
        .new-ticket-form textarea { resize: vertical; min-height: 92px; }
        .ticket-card { border-left-width: 5px; }
        .ticket-meta { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
        .ticket-chip { border-radius: 999px; padding: 6px 9px; background: #eef4ff; color: #113e86; font-size: 12px; font-weight: 900; }
        .ticket-chip.done { background: #dcfce7; color: #166534; }
        .ticket-chip.open { background: #fff7ed; color: #9a3412; }
    ...
    <header class="portal-topbar">
        <div class="brand">
            ...
            <div>
                ...
                <p>{{ \Illuminate\Support\Str::limit($empresaNome, 34) }} · acompanhe seus atendimentos com segurança.</p>
            </div>
        </div>
        <div class="top-actions">
            <div class="profile" aria-label="Cliente identificado">
                <div class="profile-avatar">{{ $iniciaisCliente }}</div>
                <div class="profile-copy">
                    <strong class="profile-name">{{ \Illuminate\Support\Str::limit($clientePublicoNome ?: 'Cliente', 28) }}</strong>
                    @if(! empty($clientePublicoEmail))
                        <span class="profile-email">{{ \Illuminate\Support\Str::limit($clientePublicoEmail, 34) }}</span>
                    @endif
                </div>
            </div>
            @if(auth('portal_cliente')->check() && \Illuminate\Support\Facades\Route::has('portal.cliente.logout'))
                <form method="POST" action="{{ route('portal.cliente.logout') }}" class="logout-form">
                    @csrf
                    <button type="submit" class="logout-button">Sair</button>
                </form>
            @else
                <a href="{{ url('/') }}" class="logout-button">Sair</a>
            @endif
        </div>
    </header>
