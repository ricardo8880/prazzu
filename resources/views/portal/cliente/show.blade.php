        ...
        <div class="portal-content">
            ...
            <section class="panel panel-pad portal-section-hidden" id="pendencias" data-portal-section="pendencias">
                <div class="panel-title-row">
                    ...
                    @if($pendenciasCount > 0)<span class="count-dot">{{ $pendenciasCount }}</span>@endif
                </div>
                ...
                <div class="pending-list">
                    @forelse($solicitacoesAbertas as $index => $solicitacao)
                        @php
                        ...
                        @endphp
                        <article class="pending-card {{ $cardClass }} {{ $index === 0 ? 'is-primary' : '' }} {{ $index > 2 ? 'is-extra-hidden' : '' }}" data-pending-card>
                            <div class="pending-head">
                                ...
                                <h3 class="pending-title">{{ $solicitacao['titulo'] ?? 'Pendência do atendimento' }}</h3>
                                <span class="priority">{{ $prioridade }}</span>
                            </div>
                            <p class="pending-text">{{ \Illuminate\Support\Str::limit($solicitacao['descricao'] ?? 'A equipe solicitou uma resposta para continuar o atendimento.', 150) }}</p>
                            <div class="pending-flow-meta">
                                <span class="deadline">
                                    ...
                                    {{ ! empty($solicitacao['created_at_label']) ? 'Aberta em: ' . $solicitacao['created_at_label'] : 'Aguardando resposta' }}
                                </span>
                                <span class="pending-order">{{ $index === 0 ? 'Comece por aqui' : 'Ordem sugerida: ' . ($index + 1) }}</span>
                            </div>
                            <button type="button" class="outline-button" style="margin-top:14px" data-toggle-pending>{{ $index === 0 ? 'Responder agora' : 'Responder pendência' }} <span>→</span></button>
                            <form method="POST" action="{{ route('portal.cliente.pendencia.responder', ['token' => $token, 'solicitacao' => $solicitacao['id']]) }}" class="mini-form js-feedback-form" data-processing="Enviando resposta...">
                                @csrf
                                ...
                                <textarea class="textarea" name="resposta" rows="3" placeholder="Escreva sua resposta para esta pendência" required>{{ old('resposta') }}</textarea>
                    ...
                    @if($solicitacoesAbertas->count() > 3)
                        <button type="button" class="all-link" data-toggle-all-pendings data-total-pendings="{{ $solicitacoesAbertas->count() }}">Ver todas as {{ $solicitacoesAbertas->count() }} pendências <span>→</span></button>
                    @else
...
<script>
    ...
    const typingUrl = '{{ route('portal.cliente.digitando', ['token' => $token]) }}';
    const chatStateUrl = '{{ route('portal.cliente.chat.estado', ['token' => $token]) }}';
